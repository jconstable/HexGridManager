...
public class HexGridManager : MonoBehaviour 
{
    ...
    {
        IntVector2 igrid = _intVectorPool.GetObject();
        PositionToGrid(pos, igrid);
        grid.x = igrid.x;
        grid.y = 0;
        grid.z = igrid.y;
        _intVectorPool.ReturnObject(igrid);
    }
    ...
    public void GetClosestVacantNeighbor( GameObject dest, ref Vector3 closestNeighbor, GameObject src = null )
    {
        IntVector2 occupantCurrent = _intVectorPool.GetObject();
        PositionToGrid(dest.transform.position, occupantCurrent);
        int currentSig = GetGridSig(occupantCurrent);

        if (!IsOccupied(currentSig))
        {
            closestNeighbor.Set( occupantCurrent.x, 0, occupantCurrent.y );
        ...
        closestNeighbor.Set( nearest.x, 0, nearest.y );
